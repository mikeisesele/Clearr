...
fun AddGoalScreen(
    ...
    onClose: () -> Unit,
    onAddGoal: (title: String, emoji: String, colorToken: String, target: String?, frequency: GoalFrequency) -> Unit,
    inferGoalDraft: suspend (
        title: String,
        target: String?,
    ...
    var aiDraft by remember { mutableStateOf<GoalsAiResult?>(null) }
    ...
    LaunchedEffect(title, target) {
        ...
        aiLoading = true
        delay(350)
        val inferred = inferGoalDraft(title, target, frequency, emoji, colorToken)
        aiDraft = inferred
        if (!targetTouched && target.isBlank() && !inferred.suggestedTarget.isNullOrBlank()) target = inferred.suggestedTarget.orEmpty()
    ...
    Column(modifier = Modifier.fillMaxSize().background(colors.bg)) {
        ClearrTopBar(title = "New Goal", onLeadingClick = onClose)
        ...
        Column(
            ...
            Button(
                onClick = {
                    onAddGoal(title.trim(), emoji, colorToken, target.trim().ifBlank { null }, frequency)
                    onClose()
                },
                enabled = canSubmit,
                modifier = Modifier.fillMaxWidth(),
