...
fun BudgetScreen(
    ...
    Box(modifier = Modifier.fillMaxSize().background(colors.bg)) {
        Column(modifier = Modifier.fillMaxSize()) {
            ClearrTopBar(
                ...
                actionText = "Edit",
                onActionClick = { onAction(BudgetAction.OpenBudgetSetup) },
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                item {
                    BudgetHeroSection(
                        ...
                        onPeriodSelect = { onAction(BudgetAction.SelectPeriod(it)) },
                        colors = colors
                    )
                }
                item {
                    BudgetCategoryTable(
                        ...
                        onCategoryDelete = { categoryId -> onAction(BudgetAction.DeleteCategory(categoryId)) },
                        onSwipeHintDisplayed = { onAction(BudgetAction.OnSwipeHintDisplayed) },
                        colors = colors
                    )
                }
    ...
    if (showLogDialog) {
        LogExpenseDialog(
            ...
            onSave = { category, amountNaira, note ->
                onAction(BudgetAction.LogExpense(category.category.id, amountNaira, note))
                showLogDialog = false
                loggingCategory = null
            }
        )
    ...
    if (state.showBudgetSetup) {
        BudgetPlanSetupDialog(
            ...
            onDismiss = { onAction(BudgetAction.DismissBudgetSetup) },
            onAmountChange = { categoryId, amountNaira -> onAction(BudgetAction.UpdateBudgetDraft(categoryId, amountNaira)) },
            onConfirm = { onAction(BudgetAction.ConfirmBudgetSetup) }
        )
    }
}
